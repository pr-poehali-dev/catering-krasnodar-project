          ...
          <div className="flex gap-4 sm:gap-6 flex-wrap">
            {STATS.map((s) => (
              <div key={s.label} className="min-w-[88px]">
                <div className="font-sans text-2xl sm:text-3xl tracking-tightest font-medium leading-none">{s.value}</div>
                <div className="text-[11px] sm:text-[12px] text-ash mt-1.5 leading-tight">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-3 sm:gap-4">
          <div className="lg:col-span-5 bento-card overflow-hidden relative min-h-[260px] sm:min-h-[320px] lg:min-h-0">
            ...
            <div className="absolute bottom-4 left-4 right-4 sm:bottom-6 sm:left-6 sm:right-6 text-snow">
              <div className="font-serif italic text-[17px] sm:text-xl leading-snug">
                «Каждый бокс собираю так, будто он для моих гостей»
              </div>
              <div className="text-[11px] sm:text-[12px] text-snow/70 mt-2 uppercase tracking-[0.15em]">
                Галина · основатель
              </div>
            </div>
          </div>
          ...
          >
            {PRINCIPLES.map((p, i) => (
              <div
                key={p.title}
                className="bento-card bg-snow p-5 sm:p-6 transition-all duration-[900ms] ease-[cubic-bezier(0.16,1,0.3,1)]"
                style={{
              ...
              >
                <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-lime/25 flex items-center justify-center mb-4">
                  <Icon name={p.icon} size={20} className="text-graphite" />
                </div>
                <h3 className="font-sans text-[16px] sm:text-[17px] font-medium tracking-tight mb-1.5">
                  {p.title}
                </h3>
                <p className="text-[13px] sm:text-[14px] text-graphite/70 leading-relaxed">{p.text}</p>
              </div>
            ))}
          </div>
        </div>
