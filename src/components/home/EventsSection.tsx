  ...
  return (
    <section id="events" className="pt-10 sm:pt-14 lg:pt-16 pb-16 sm:pb-24 lg:pb-28 relative scroll-mt-24">
      <div className="container mx-auto">
        ...
        <div ref={grid.ref as never} className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
          {events.map((e, i) => (
            <Link
              key={e.id}
              to={`/events/${e.id}`}
            ...
            >
              ...
              <div className="absolute top-1.5 right-1.5 sm:top-4 sm:right-4">
                <span className="glass-dark text-snow text-[7px] sm:text-[11px] uppercase tracking-wider px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-full border hairline-light">
                  {e.badge}
                </span>
              </div>
              <div className="absolute bottom-2 left-2 right-2 sm:bottom-5 sm:left-5 sm:right-5 flex items-end justify-between text-snow gap-1.5">
                <div className="min-w-0">
                  <div className="font-sans text-[13px] sm:text-2xl lg:text-3xl tracking-tighter font-medium leading-tight truncate">{e.title}</div>
                  <p className="hidden sm:block text-[12px] text-snow/70 mt-1 leading-snug line-clamp-2">{e.subtitle}</p>
                  <div className="text-[9px] sm:text-[13px] text-snow/80 mt-0.5 sm:mt-1 truncate">{e.price}</div>
                </div>
                <div className="w-6 h-6 sm:w-10 sm:h-10 rounded-full bg-lime text-graphite flex items-center justify-center shrink-0 sm:opacity-0 sm:group-hover:opacity-100 sm:translate-y-2 sm:group-hover:translate-y-0 transition-all">
